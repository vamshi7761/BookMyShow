...
@CommandType(action = "DELETE", entity = "SHOW_SEAT_TYPE")
public class DeleteShowSeatTypeCommandHandler implements CommandHandler {

    private final ShowSeatTypeService showSeatTypeService;

    @Override
    public OutputCommand execute(InputCommand request) {
        return showSeatTypeService.deleteShowSeatType(request);
    }
}
